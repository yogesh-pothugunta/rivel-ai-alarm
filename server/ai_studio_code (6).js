 ...
 * 
 * Production-ready backend for Google Cloud Run / Container Deployment.
 * 
...
function normalizeModelName(rawModel) {
  ...
  // If undefined string, empty, or legacy/pro models (which have 0 free tier quota), use default
  if (!model || model === 'undefined' || model === 'null' || model === 'gemini-2.5-flash' || model === 'gemini-2.5-pro' || model === 'gemini-3.1-pro-preview' || model === 'gemini-2.0-flash' || model === 'gemini-1.5-flash' || model === 'gemini-1.5-pro') {
...
const GEMINI_TIMEOUT_MS = parseInt(process.env.GEMINI_TIMEOUT_MS || '35000', 10);
...
async function callGeminiApi(payload, preferredModel = CONFIGURED_MODEL) {
  ...
  // Candidate models to attempt: preferred model, then known stable lightweight fallback models
  const modelsToAttempt = [preferredModel];
  if (!modelsToAttempt.includes('gemini-3.6-flash')) modelsToAttempt.push('gemini-3.6-flash');
  if (!modelsToAttempt.includes('gemini-3.1-flash-lite')) modelsToAttempt.push('gemini-3.1-flash-lite');
  if (!modelsToAttempt.includes('gemini-3.5-flash-lite')) modelsToAttempt.push('gemini-3.5-flash-lite');
  ...
  const perModelTimeout = Math.max(20000, Math.floor(GEMINI_TIMEOUT_MS / 2));
...
app.post('/api/vision/verify-object', visionLimiter, async (req, res) => {
  try {
    ...
    const prompt = `You are a physical wake-up alarm verification AI.
...
1. Identify if the photo clearly shows a "${cleanTarget}" (or common everyday variations):
   - "Book" / "Notebook" -> book, textbook, spiral notebook, novel, diary, study journal, booklet.
   - "Keys" -> house keys, room keys, car/bike keys, key fob, keychain with keys.
   - "Spoon" -> metal spoon, plastic spoon, tea spoon, dessert spoon, soup spoon, eating utensil.
   - "Water Bottle" / "Cup" -> water bottle, flask, tumbler, hydroflask, coffee mug, tea cup, drinking glass.
   - "Toothbrush" -> manual toothbrush, electric toothbrush head.
   - "Shoe" -> sneaker, running shoe, slipper, sandal, boot, formal shoe.
   - "Pen" -> pen, ballpoint, gel pen, pencil, highlighter, marker.
   - "Headphones" -> over-ear headphones, on-ear headphones, earbuds in/out of case.
   - "Watch" -> wristwatch, smartwatch, fitness band.
   - "Glasses" -> spectacles, reading glasses, sunglasses.
   - "Backpack" -> school bag, backpack, rucksack, laptop bag.
   - "Chair" / "Pillow" -> chair, office chair, stool, bed pillow, cushion.
2. If the user photographed a completely different object or plain empty wall/floor, set detected to false.
3. If the photo is completely pitch black or totally obstructed, set detected to false.

You MUST respond ONLY with a valid JSON object in this exact schema:
{
  "detected": true,
  "confidence": 0.95,
  "reason": "A ${cleanTarget} is clearly visible in the photo."
}`;

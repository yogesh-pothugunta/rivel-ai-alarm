...
// Active fast Gemini models supported in current environment
...
function normalizeModelName(rawModel) {
  ...
  if (!model || model === 'undefined' || model === 'null' ||
      ...
      model === 'gemini-1.5-pro' || model === 'gemini-2.0-pro' ||
      model === 'gemini-3.6-flash' || model === 'gemini-2.5-flash-lite') {
...
app.post(['/api/vision/verify-object', '/api/verify-object'], async (req, res) => {
  try {
    ...
    // Fast, comprehensive everyday household object recognition prompt
    const prompt = `Task: Is the target object "${targetObject}" present in this photo?
Target object: "${targetObject}"

Everyday Household Variations Recognized:
- Water Bottle: flask, tumbler, hydro flask, plastic bottle, metal water bottle.
- Coffee Mug / Cup: mug, tea cup, glass, paper cup, ceramic cup.
- Spoon / Fork / Plate: spoon, fork, teaspoon, plate, bowl, kitchen dish.
- Refrigerator: fridge, freezer door, kitchen refrigerator.
- Toothbrush / Toothpaste: manual toothbrush, electric toothbrush, toothpaste tube.
- Bathroom Mirror / Sink: washroom mirror, basin, bathroom faucet.
- Soap / Towel: soap bar, liquid handwash dispenser, bath towel, hand towel.
- Water Tap: sink faucet, tap, water dispenser.
- Book / Notebook: textbook, novel, notebook, journal, syllabus book, papers.
- Pen: ballpoint pen, ink pen, pencil, highlighter, marker.
- Backpack: study backpack, rucksack, college bag, shoulder bag.
- Laptop / Keyboard: notebook computer, keyboard, mouse, tablet, screen.
- Desk Lamp: table lamp, study light.
- Chair / Table: study chair, stool, office chair, desk, table.
- Shoes / Slippers: sneaker, shoes, footwear, slippers, flip-flops, sandals.
- Keys: house key, car key, key chain, bunch of keys.
- Glasses: spectacles, eyeglasses, reading glasses, sunglasses.
- Watch: wristwatch, smartwatch, fitness band, wall clock.
- Headphones: over-ear headphones, earphones, earbud charging case.
- Door Handle: door knob, latch, room door, main entrance handle.
- Pillow: bed pillow, cushion, blanket, bedsheet.
- Clothes Hanger: wardrobe hanger, coat hanger, closet.

Respond strictly in this compact JSON format:
{"detected":true/false,"confidence":0.95,"reason":"short reason"}`;
    ...
    if (result.status !== 200) {
      return res.status(result.status).json({
        ...
        isDetected: false,
        confidence: 0.0,
        reason: result.data?.error?.message || 'Gemini Vision AI busy. Please retry.'
      });
    }

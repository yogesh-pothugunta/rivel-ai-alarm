...
// Helper: Call Gemini API securely with timeout and model fallback
async function callGeminiApi(payload, preferredModel = CONFIGURED_MODEL) {
  ...
  for (let i = 0; i < modelsToAttempt.length; i++) {
    ...
    try {
      ...
      // If status is 404 (model not found) and we have fallback models left, continue to next candidate
      if (response.status === 404 && i < modelsToAttempt.length - 1) {

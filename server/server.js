...
// gemini-2.5-flash is the primary stable multimodal workhorse for vision tasks
...
function normalizeModelName(rawModel) {
  ...
  if (!model || model === 'undefined' || model === 'null' || model === 'gemini-2.0-flash' || model === 'gemini-1.5-flash' || model === 'gemini-1.5-pro' || model === 'gemini-2.5-pro' || model === 'gemini-3.1-pro-preview') {
...
async function callGeminiApi(payload, preferredModel = CONFIGURED_MODEL) {
  ...
  if (!modelsToAttempt.includes('gemini-2.5-flash-lite')) modelsToAttempt.push('gemini-2.5-flash-lite');
  ...
  const perModelTimeout = 18000; // 18 seconds per attempt

...
function normalizeModelName(rawModel) {
  ...
  if (!model || model === 'undefined' || model === 'null' || model === 'gemini-2.5-flash' || model === 'gemini-2.5-pro' || model === 'gemini-2.0-flash' || model === 'gemini-1.5-flash' || model === 'gemini-1.5-pro') {
...
async function callGeminiApi(payload, preferredModel = CONFIGURED_MODEL) {
  ...
  if (!modelsToAttempt.includes('gemini-3.1-pro-preview')) modelsToAttempt.push('gemini-3.1-pro-preview');
